...
#26 jazz  0 classic 73 calm 118 hiphop 5 pop
def decode(encoded, genre):
    midi_out = pretty_midi.PrettyMIDI()
    if genre=='classic':
        midi_out.instruments.append(pretty_midi.Instrument(0, name='Piano'))
    if genre=='jazz':
        midi_out.instruments.append(pretty_midi.Instrument(26, name='Electric Guitar (jazz)'))
    if genre=='calm':
        midi_out.instruments.append(pretty_midi.Instrument(73, name='Flute'))
    if genre=='pop':
        midi_out.instruments.append(pretty_midi.Instrument(5, name='Electric Piano 2'))
    if genre=='hiphop':
        midi_out.instruments.append(pretty_midi.Instrument(118, name='Synth Drum',is_drum=True))
...
def filter_bad_note_offs(events):
    """Очистить события NOTE_OFF, для которых отсутствует соответствующее событие NOTE_ON.."""
    notes_down = {}  # pitch: 1
    ...
    for i, ev in enumerate(events):
        ...
        if typ == NOTE_OFF:
            ...
            if notes_down.get(pitch) is None:
                # если NOTE_OFF без NOTE_ON, то удалить событие
                keep_idxs.remove(i)
            else:
